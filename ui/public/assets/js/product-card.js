...
const addToCart = (name, image, price, productId, category) => {
    ...
    let payload = {productId, quantity: 1, location: category, tableNumber: 1 };
    makeOrder(payload);
    console.log(payload);
...
poolSofts = existingProducts.filter(item => item.type === 'soft drinks');
poolWines = existingProducts.filter(item => item.type === 'wines');
poolAlcohols = existingProducts.filter(item => item.type === 'alcohols');
poolBeers = existingProducts.filter(item => item.type === 'beers');
poolCocktails = existingProducts.filter(item => item.type === 'cocktails');
...
const card = (product) => `
            ...
            <div class="product-wrapper mb-25">
                <div class="product-img">
                    ...
                    <div class="product-action">
                        <div class="pro-action-left">
                            <a style="cursor: pointer;" onclick="addToCart('${product.name}', '${product.imageUrl}', '${product.price}', '${product.id}', '${product.category}')" title="Add To Cart"><i class="ion-android-cart"></i> Add To Cart</a>
                        </div>
                ...
                <div class="product-content">
                    <h4>
                        <a style="cursor: auto;" href="#product-details">${product.name}</a>
                    </h4>
...
// async function addProducts(payload, category, type) {
...
//         const data = await res.json();
//     console.log(data.data);
//     localStorage.setItem('products', JSON.stringify([...existingProducts, data.data]));
// }
...
//     image: 'assets/images/lounge/Power-Horse.jpg',
//     price: '1200'
// }].forEach(product => addProducts(product, 'poolside', 'cocktails'));

// poolSofts.forEach(product => addProducts(product, 'poolside', 'soft drinks'));
// poolWines.forEach(product => addProducts(product, 'poolside', 'wines'));
// poolAlcohols.forEach(product => addProducts(product, 'poolside', 'alcohols'));
// poolBeers.forEach(product => addProducts(product, 'poolside', 'beers'));
// poolCocktails.forEach(product => addProducts(product, 'poolside', 'cocktails'));
